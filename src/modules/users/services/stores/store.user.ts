...
import { ModelUser, ModelAddress, ModelPushOneSignal, TypeGender, TypeDevice } from '../../models/index';
...
export class StoreUser {
    ...
    createByPhoneNumber(phoneNumber:string, typeDevice: TypeDevice): Promise<ModelUser> {
        return new Promise<ModelUser>(async (resolve, reject) => {
            const user = new ModelUser();
            user.phoneNumber = phoneNumber;
            user.typeDevice = typeDevice;
    ...
    createByEmail(email: string, password: string, typeDevice: TypeDevice): Promise<ModelUser> {
        return new Promise<ModelUser>(async (resolve, reject) => {
            const user = new ModelUser();
            user.email = email;
    ...
    createFull(firstname: string, lastname: string, email: string, password: string, birthDate: string,
        photoUrl: string, facebookId: string, facebookAccessToken: string,
        gender: TypeGender, typeDevice: TypeDevice, phoneNumber:string): Promise<ModelUser> {
        return new Promise<ModelUser>(async (resolve, reject) => {
            const user = new ModelUser();
    ...
    getByPhoneNumber(phoneNumber: string): Promise<ModelUser> {
        return new Promise<ModelUser>((resolve, reject) => {
            this.mongoClient.findOneByFilter(this.collectionName, { phoneNumber: phoneNumber }, (error, data: ModelUser) => {
                if (error)
                    reject(error);
                else
                    resolve(data);
    ...
    getByPhoneNumberVerificationId(phoneNumber: string, phoneNumberVerificationId:string): Promise<ModelUser> {
        return new Promise<ModelUser>((resolve, reject) => {
            this.mongoClient.findOneByFilter(this.collectionName, { phoneNumber: phoneNumber, phoneNumberVerificationId: phoneNumberVerificationId }, (error, data: ModelUser) => {
                if (error)
                    reject(error);
                else
                    resolve(data);
    ...
    edit(id: string, firstname: string, lastname: string, birthDate: string, phoneNumber: string, gender: TypeGender,
    ...
    setPhoneNumberVerificationId(id: string): Promise<ModelUser> {
        return new Promise<ModelUser>(async (resolve, reject) => {
            const user = await this.get(id);
            user.phoneNumberVerificationId = parseInt(this.generateIdPhoneNumber(6));
    ...
    setPhoneNumberVerificationIdByPhoneNumber(phoneNumber: string): Promise<ModelUser> {
        return new Promise<ModelUser>(async (resolve, reject) => {
            const user = await this.getByPhoneNumber(phoneNumber);
            user.phoneNumberVerificationId = parseInt(this.generateIdPhoneNumber(6));
            user.phoneNumberVerified = false;
            this.mongoClient.update(this.collectionName, user._id, user, (error, data: ModelUser) => {
                if (error)
                    reject(error);
                else
                    resolve(data);
    ...
    setConsentVersion(id: string, consentVersion: number): Promise<ModelUser> {
        return new Promise<ModelUser>(async (resolve, reject) => {
            const user = await this.get(id);
    ...
    setPhoneNumberVerified(id: string): Promise<ModelUser> {
        return new Promise<ModelUser>(async (resolve, reject) => {
            const user = await this.get(id);
            user.phoneNumberVerified = true;
            user.phoneNumberVerificationId = null;
            this.mongoClient.update(this.collectionName, id, user, (error, data: ModelUser) => {
                if (error)
                    reject(error);
                else
    ...
    setPushToken(userId: string, token: string, userIdOneSignal): Promise<ModelUser> {
        return new Promise<ModelUser>(async (resolve) => {
            let user = await this.get(userId);
            user.push = new ModelPushOneSignal();
            user.push.pushToken = token;
            user.push.userIdOneSignal = userIdOneSignal;
            this.mongoClient.update(this.collectionName, userId, user, (error, data) => {
                resolve(data);
            });
        });
    }
