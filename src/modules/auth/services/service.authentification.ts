...
import { CreateUserByPhoneNumberRequest } from "../../users/models";
...
export class ServiceAuthentification {
  ...
  async signupByPhoneNumber(
    newUser: PhoneNumberAuthRequest,
    host: string,
  ): Promise<IModelResponse> {
    try {
      newUser.phoneNumber = newUser.phoneNumber.trim();
      const phoneNumber = parsePhoneNumber(newUser.phoneNumber, "FR");

      newUser.phoneNumber = phoneNumber.formatInternational();
      let accesTokenAuthResponse = new AccesTokenAuthResponse();
      let user = await this.storeUser.getByPhoneNumberVerificationId(
        newUser.phoneNumber,
        newUser.PhoneNumberVerificationId,
      );
      if (!user) {
        return (ResponseFailure(
          400,
          `User with this phone number ${newUser.phoneNumber} does not exist for this verification id`,
        ));
      } else {
        user = await this.storeUser.setPhoneNumberVerified(user._id);
        if (!user) {
          return ResponseFailure(
            500,
            `Something wrong for this user with this phone number ${newUser.phoneNumber.toLowerCase()}`,
          );
        } else {
          const payload = {
            admin: user.isAdmin,
          ...
          const token = jwt.sign(payload, configAuth.secret, {
            expiresIn: 60 * 60 * 24 * 1095, // expires in 30 days
          });
