...
import { CreateUserRequest, FindUserByEmailRequest, ServiceUser, UserResponse, FindUserByEmailVerificationIdRequest, FindUserByPasswordVerificationIdRequest, CreateUserByPhoneNumberRequest } from '../../../modules/users/index';
...
export class ControllerAuthentification {
    ...
    @ApiOperationPost({
        path: '/sendVerificationIdByPhoneNumber',
        description: "Send Verification Id By PhoneNumber",
        summary: "Send Verification Id By PhoneNumbe",
        parameters: {
            body: { description: "sendVerificationIdByPhoneNumber", required: true, model: "CreateUserByPhoneNumberRequest" }
        },
    ...
    @httpPost('/sendVerificationIdByPhoneNumber', validateBody(CreateUserByPhoneNumberRequest))
    public async sendVerificationIdByPhoneNumber(req: Request, res: Response) {
        appInsights.defaultClient.trackNodeHttpRequest({ request: req, response: res });
        try {
            const resp = await this.serviceUser.sendVerificationIdByPhoneNumber(req.body);
            if (!resp) {
                res.status(400).send(`Object AuthResponse is null`);
            }
            else
    ...
    public async forgetPassword(req: Request, res: Response) {
        ...
        try {
            if ((req.protocol + "://" + req.get('host')) == ("https://" + req.get('host'))) {
                ...
                const resp = await this.serviceUser.sendPasswordVerification(req.body, req.get('host'));
                if (!resp) {
                    res.status(400).send(`Object UserResponse is null`);
                }
                else
