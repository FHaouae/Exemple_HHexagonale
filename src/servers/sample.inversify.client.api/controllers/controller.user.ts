...
import { ServiceUser, FindUserByIdRequest, UserResponse, FindUserByEmailRequest, EditUserRequest, EditUserPasswordRequest, EditUserEmailRequest, EditUserFacebookInformationsRequest, EditUserMobileTokensRequest, EditUserPhotoByIdRequest, UpdatePushOneSignalByUserIdRequest, EditUserConsentVersionByIdRequest } from '../../../modules/users/index';
...
export class ControllerUser {
    ...
    @ApiOperationPut({
        path: '/update/consentVersion',
        description: "Update consentVersion of user by userId",
        summary: "Update consentVersion of user by userId",
        parameters: {
            body: { description: "Update consentVersion of user by userId", required: true, model: "EditUserConsentVersionByIdRequest" }
        },
    ...
    @httpPut('/update/consentVersion', validateBody(EditUserConsentVersionByIdRequest))
    public async setConsentVersion(req: Request, res: Response) {
        appInsights.defaultClient.trackNodeHttpRequest({ request: req, response: res });
        try {
            const token = req.headers.authorization.split(' ')[1];
            const decodedToken = jwt.verify(token, config.secret);
            if (decodedToken.userId != req.body.userId) {
                ...
                const resp = await this.serviceUser.setConsentVersion(req.body);
                if (!resp) {
                    res.status(400).send(`Object UserResponse is null`);
                }
                else
    ...
    @ApiOperationPut({
        path: '/push/oneSignal',
        description: "Push One Signal by userId",
        summary: "Push One Signal by userId",
        parameters: {
            body: { description: "Push One Signal by userId", required: true, model: "UpdatePushOneSignalByUserIdRequest" }
        },
    ...
    @httpPut('/push/oneSignal', validateBody(UpdatePushOneSignalByUserIdRequest))
    public async setPushToken(req: Request, res: Response) {
        ...
        try {
            ...
            if (decodedToken.userId != req.body.userId) {
                ...
                const resp = await this.serviceUser.setPushToken(req.body);
                if (!resp) {
                    res.status(400).send(`Object UserResponse is null`);
                }
                else
